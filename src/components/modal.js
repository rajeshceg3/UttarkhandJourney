export const showModal = (location) => {
    ...
    modal.innerHTML = `
        <div class="modal-content bg-white p-6 rounded-lg shadow-xl w-11/12 max-w-lg relative animate-fade-in">
            <button class="modal-close-btn absolute top-4 right-4 text-gray-500 hover:text-gray-800" aria-label="Close modal">
                <i data-feather="x" width="24" height="24"></i>
            </button>
            <h2 class="text-2xl font-serif font-bold mb-4">${location.title}</h2>
            <img src="${location.image}" alt="${location.title}" class="w-full h-48 object-cover rounded-md mb-4" loading="lazy">
